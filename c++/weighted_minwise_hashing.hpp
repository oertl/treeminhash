  ...
  operator()(const std::vector<std::pair<uint64_t, double>> &data,
    ...
    const double weightSum =
        std::accumulate(data.begin(), data.end(), 0.,
                        [](double x, const auto &d) { return x + d.second; });
    ...
    while (true) {
      ...
      for (const auto &[d, w] : data) {
        ...
        while (true) {
          while (nodeIdx < numNonLeafNodes &&
                 tree[nodeIdx].lowerBound < w) { // while not leaf do
            ...
            for (uint32_t k = 0; k < m; ++k) {
              ...
              if (point < result[idx].second) {
                if (!(acceptanceProbability < 1)) {
                  ...
                  do {
                    bool accept =
                        getUniformDouble(rng2) < acceptanceProbability;
                    if (accept) {
                      result[idx] = {d, point};
                      break;
                    }
      ...
      bool success =
          std::none_of(result.begin(), result.end(),
                       [limit](const auto &r) { return r.second == limit; });
      ...
      std::for_each(result.begin(), result.end(), [oldLimit, limit](auto &d) {
        if (d.second == oldLimit)
          d.second = limit;
      });
    }
  }
