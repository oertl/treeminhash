...
template<typename S>
void testCase(const tmh::Weights& w, const string& algorithmDescription, uint32_t m, uint64_t numIterations, S&& hashFunctionSupplier) {
    ...
    #pragma omp parallel
    {
        ...
        for (uint64_t i = 0; i < numIterations; ++i) {
            ...
            const pair<vector<pair<uint64_t,double>>,vector<pair<uint64_t,double>>> data = generateData(rng, w);
...
void testCase(const tmh::Weights& w, uint32_t hashSize, uint64_t numIterations) {

    testCase(w, "BagMinHash2", hashSize, numIterations, [](uint32_t m){return [m](const vector<pair<uint64_t, double>>& x){return bmh::bag_min_hash_2<bmh::FloatWeightDiscretization, bmh::XXHash64>(x,m).hashValues;};});
    testCase(w, "DartMinHash", hashSize, numIterations, [](uint32_t m){mt19937_64 r(UINT64_C(0x1e402663a58575d1)); return dmh::DartMinHash(r, m);});
    testCase(w, "TreeMinHash", hashSize, numIterations, [](uint32_t m){return tmh::TreeMinHash<RNGFunction>(m, RNGFunction(UINT64_C(0x8eddaf1bc709729d)));});
