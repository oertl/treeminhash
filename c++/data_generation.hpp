...
#ifndef _TMH_DATA_GENERATION_HPP_
#define _TMH_DATA_GENERATION_HPP_
...
public:
  ...
  std::tuple<size_t, size_t, size_t> getSizes() const {
    ...
    for (const auto &w : weights) {
      ...
      if (std::get<1>(w) > 0)
        sizeB += 1;
      if (std::get<0>(w) > 0 && std::get<1>(w) > 0)
        sizeAB += 1;
    }
  ...
  const std::string &getLatexDescription() const { return latexDescription; }

  const std::string &getId() const { return id; }

  Weights(const std::vector<std::tuple<double, double>> &weights,
          const std::string &latexDescription, const std::string &id)
      : weights(weights), latexDescription(latexDescription), id(id) {}

  bool allWeightsZeroOrOne() const {
    for (const auto &w : weights) {
      if ((std::get<0>(w) != 0 && std::get<0>(w) != 1) ||
          (std::get<1>(w) != 0 && std::get<1>(w) != 1))
        return false;
    }
    return true;
  }
...
template <typename R>
std::pair<std::vector<std::pair<uint64_t, double>>,
          std::vector<std::pair<uint64_t, double>>>
generateData(R &rng, const Weights &w) {
  ...
  for (const auto &x : w.getWeights()) {
    uint64_t data = rng();
    if (std::get<0>(x) > 0)
      valuesA.emplace_back(data, std::get<0>(x));
    if (std::get<1>(x) > 0)
      valuesB.emplace_back(data, std::get<1>(x));
  }

  std::shuffle(valuesA.begin(), valuesA.end(), rng);
  std::shuffle(valuesB.begin(), valuesB.end(), rng);

  return std::make_pair(valuesA, valuesB);
}
...
Weights getWeightsCase_83f19a65b7f42e88() {
  ...
  return Weights(v,
                 "$\\bigcup_{\\symTestCaseIndex=0}^{1000} "
                 "\\lbrace({1.001}^\\symTestCaseIndex, "
                 "{1.002}^{\\symTestCaseIndex})\\rbrace$",
                 "83f19a65b7f42e88");
}
...
Weights getWeightsCase_0a92d95c38b0bec5() {
  ...
  return Weights(v, "$\\lbrace(1,3)^{300},(2,1)^{500},(5,4)^{700}\\rbrace$",
                 "0a92d95c38b0bec5");
}
...
Weights getWeightsCase_a9415c152258dac1() {
  ...
  return Weights(v,
                 "$\\bigcup_{\\symTestCaseIndex=0}^{1000} "
                 "\\lbrace({0.999}^\\symTestCaseIndex, "
                 "{1.001}^{\\symTestCaseIndex})\\rbrace$",
                 "a9415c152258dac1");
}
...
Weights getWeightsCase_431c7f212064fc5d() {
  ...
  return Weights(
      v,
      "$\\bigcup_{\\symTestCaseIndex=0}^{1000} \\lbrace(\\symTestCaseIndex, "
      "1000-{\\symTestCaseIndex})\\rbrace$",
      "431c7f212064fc5d");
}
...
Weights getWeightsCase_8d6bb210472266c3() {
  ...
  return Weights(v,
                 "$\\bigcup_{\\symTestCaseIndex=0}^{500} "
                 "\\lbrace({0.98}^\\symTestCaseIndex, "
                 "{1.01}^{\\symTestCaseIndex})\\rbrace$",
                 "8d6bb210472266c3");
}
...
Weights getWeightsCase_8a224349623eeb24() {
  ...
  return Weights(v,
                 "$\\bigcup_{\\symTestCaseIndex=0}^{300} "
                 "\\lbrace({0.96}^\\symTestCaseIndex, "
                 "{1.01}^{\\symTestCaseIndex})\\rbrace$",
                 "8a224349623eeb24");
}
...
#endif // _TMH_DATA_GENERATION_HPP_
