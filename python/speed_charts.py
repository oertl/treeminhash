...
def drawChart(ax, data, hashSize, mode, isLastRow, isFirstCol):
    ...
    for algorithm in algorithms:
        ...
        ax.plot(
            ...
            y,
            marker=".",
            label=algorithm.translate(str.maketrans({"_": r"\_"})),
            color=color_defs.colors[algorithm],
            linewidth=1,
        )
...
for i in range(0, len(hashSizes)):
    for j in range(0, len(modes)):
        drawChart(
            ax[i][j], data, hashSizes[i], modes[j], i + 1 == len(hashSizes), j == 0
        )

fig.subplots_adjust(
    left=0.025, bottom=0.045, right=0.994, top=0.975, wspace=0.05, hspace=0.15
)
fig.savefig(
    "paper/speed_charts.pdf", format="pdf", dpi=1200, metadata={"creationDate": None}
)
fig.savefig("paper/speed_charts.svg", format="svg", dpi=1200)
plt.close(fig)
